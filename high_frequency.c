...
void imgHighFrequency3(ImgMat *src,ImgMat *dst)
{
	#ifdef DEBUG
	SOURCE_ERROR_CHECK(imgHighFrequency,src);
	DESTINATION_ERROR_CHECK(imgHighFrequency,dst);
	#endif
	...
	int dst_type;
	dst_type = (src->type)&0x01;
	
	if((dst->height != img_height)||(dst->width != img_width)||(dst->type != dst_type))
	...
	for(k=0;k<cn;k++)
	{
		...
		for(j=1;j<img_height-1;j++)
		{
			...
			for(i=1;i<img_width-1;i++)
			{
				...
				dst_data[i] = src_data0[i]-(sum0+sum1+sum2+4)/9;
			}
			dst_data[i] = 0;
		}
...
void imgHighFrequency5(ImgMat *src,ImgMat *dst)
{
	#ifdef DEBUG
	SOURCE_ERROR_CHECK(imgHighFrequency,src);
	DESTINATION_ERROR_CHECK(imgHighFrequency,dst);
	...
	for(k=0;k<cn;k++)
	{
		...
		for(j=2;j<img_height-2;j++)
		{
			...
			for(i=2;i<img_width-2;i++)
			{
				...
				dst_data[i] = src_data0[i]-(sum0+sum1+sum2+sum3+sum4+12)/25;
			}
...
void imgHighFrequency(ImgMat *src,ImgMat *dst,int r)
{
	if(r == 1)
	{
		imgHighFrequency3(src,dst);
		return;
	}
	
	if(r == 2)
	{
		imgHighFrequency5(src,dst);
		return;
	}
	...
	for(k=0;k<cn;k++)
	{
		...
		for(j=r;j<img_height-r;j++)
		{
			...
			for(i=r;i<img_width-r;i++)
			{
				...
				DST(i,j) = SRC(i,j)-(data_sum+(area>>1))/area;		
			}
...
void HighFrequency(ImgMat *src,ImgMat *dst,int r)
{
	#ifdef DEBUG
	SOURCE_ERROR_CHECK(imgHighFrequency,src);
	#endif
